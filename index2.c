...
int main(int argc, char *argv[]){
		...
		if (eerror==1) {
			...
			eerror=0;
			system("nasm -o out.exe index.dat 2> indexx.dat");
...
void head(){
			...
			addcode ("[bits 16]");
			addcode ("org 0h");
			addcode ("host_exe_header:");
			addcode (".signature: db 'MZ'     ; the 'MZ' characters");
			...
			addcode (".in_cs: dw  0      ; initial relative value of the text segment");
			addcode (".offset: dw 0         ; offset of the relocation table from start of header");
